...
from ecgutils import plot_confusion_mat, plot_cam_background_median, plot_roc
...
if MODEL_LOAD is False:
    model_lvh = Sequential()

    model_lvh.add(Conv1D(filters=64,
                     kernel_size=10,
                     input_shape=x_train[0].shape, 
                     activation = "relu"))
    
    model_lvh.add(Conv1D(filters=32,
                     kernel_size=10,
                     dilation_rate=3,
                     activation='relu'))
    ...
    #model_lvh.add(Flatten())
    model_lvh.add(GlobalAveragePooling1D())
    #model_lvh.add(Dense(128, activation = 'relu'))
    #model_lvh.add(Dropout(0.2))
...
y_pred_sbrad = model_lvh.predict(x_test)
...
y_clf = glostrup_baseline.s_peak_amp_v1.values + np.maximum(glostrup_baseline.r_peak_amp_v5.values,
                                                          glostrup_baseline.r_peak_amp_v6.values)
...
layer_idx = -1
model_lvh.layers[layer_idx].activation = activations.linear
model_lvh = utils.apply_modifications(model_lvh)
#%% Get expample from test_id
n = get_n(testid=glostrup.testid.iloc[20], df=glostrup, index=train_index)

example = (x_train[n, :, :]).astype(dtype=float)
#%% Plot
cam = visualize_cam(model_lvh, layer_idx, filter_indices=1, seed_input=example, 
                    penultimate_layer_idx=None, backprop_modifier=None, grad_modifier=None)

plot_cam_background_median(example, cam, glostrup, "LVH", y_train[n])
