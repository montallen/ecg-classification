...
if MODEL_LOAD is False:
    ...
    model_sex.add(Conv1D(filters=64,
                     kernel_size=10,
                     input_shape=x_train[0].shape, 
                     activation = "relu"))
    
    model_sex.add(Conv1D(filters=32,
                     kernel_size=10,
                     dilation_rate=3,
                     activation='relu'))
    ...
    #model_sex.add(Flatten())
    model_sex.add(GlobalAveragePooling1D())
    #model_sex.add(Dense(128, activation = 'relu'))
    #model_sex.add(Dropout(0.2))
    
    model_sex.add(Dense(2, activation='sigmoid'))
#   

    weights = {True: 3,
               False: 1}
    
    model_sex.compile(loss='categorical_crossentropy',
                  optimizer=Adam(lr=0.0012,),
                  metrics=['accuracy'])

    history = model_sex.fit(x_train, y_train, 
                        batch_size=64, 
                        epochs=100, 
...
model_sex = load_model(WDIR + "source/src/mdl/gender_cnn.hdf5")
...
y_pred_sbrad = model_sex.predict(x_test)
...
X_clf = glostrup_baseline[["bmi","qrs","qt","pr","p_peak_amp_v5",
...
n = get_n(testid=glostrup.testid.iloc[700], df=glostrup, index=test_index)
...
plot_cam_background_median(example, cam, glostrup, "Gender", y_test[n])
